...
def load_archived_murs(mur_no=None):
    ...
    if LOCAL_DEBUG:
        print("Loading archived mur{0}...".format(mur_no))
...
def extract_pdf_text(mur_no=None):
    ...
    if LOCAL_DEBUG:
        print("Get {0} archived mur(s) from elasticserch index: \"archived_murs\"".format(
            str(len(results['all_mur_docs']))))
        ...
        with db.engine.connect() as conn:
            for mur in results["all_mur_docs"]:
                ...
                if mur.get("documents"):
                    ...
                    if LOCAL_DEBUG:
                        print("Archived Mur{0} has documents.".format(mur_no))
                    ...
                    if LOCAL_DEBUG:
                        print("Archived Mur{0} has no documents.".format(mur_no))
                    insert_data_doc = ({
                        "document_id": None,
                        "mur_no": mur_no,
                        "pdf_text": None,
